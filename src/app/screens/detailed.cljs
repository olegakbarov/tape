...
(defn DetailsContent
  ...
  (let [[market pair] (:ui/detailed-view @db)
        ...
        {:keys [high low sell buy currency-pair market timestamp avg last vol
                vol-cur]}
          content
    ...
    [:div
     [:div#detailed
      [:div.header
       ...
       [:div.close {:on-click #(close-detailed-view)}]] [:div.market " " market]
      [:div.labels [:div.item "High"] [:div.item "Low"] [:div.item "Buy"]
       [:div.item "Sell"]]
      [:div.prices.last [:div.item (when high (js/parseInt high))]
       [:div.item (when low (js/parseInt low))]
       [:div.item (when buy (js/parseInt buy))]
       [:div.item (when sell (js/parseInt sell))]]] [Chart]]))
